...
local clock_widget = awful.widget.textclock("%a %b %d, %H:%M:%S", 1)
...
awful.screen.connect_for_each_screen(function(s)
	...
	local w = wibox.widget({ widget = s.mytaglist })
...
local globalkeys = gears.table.join(
	...
	awful.key({ modkey }, "s", function()
		awful.spawn(home .. "/.config/rofi/launch.sh")
	...
	end, { description = "use layout 1", group = "layout" }),

	awful.key({ modkey, "Control" }, "s", function()
		awful.layout.set(awful.layout.layouts[2])
	end, { description = "use layout 2", group = "layout" }),
